...
import type { CreateTransactionDto, UpdateTransactionDto } from '@bills/types'
...
export function TransactionForm() {
  ...
  useEffect(() => {
    if (editingTx) {
      setTxType(editingTx.type as TxType)
      setForm({
        description: editingTx.description,
        amount: String(Number(editingTx.amount)),
        currency: editingTx.currency,
        accountId: editingTx.accountId,
        toAccountId: (editingTx as any).toAccountId ?? '',
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editingTx) {
        ...
        await updateMutation.mutateAsync({ id: editingTx.id, dto })
      } else {
      ...
      <div className="bg-card border border-border rounded-xl shadow-2xl w-full max-w-md max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="px-6 py-5 space-y-4">
          ...
          <div className="space-y-1">
            ...
            <Input value={form.description} onChange={set('description')} placeholder="e.g. Grocery shopping" required />
          </div>
          ...
          <div className="space-y-1">
            ...
            <select value={form.accountId} onChange={set('accountId')}
              className="w-full h-10 px-3 rounded-md border border-input bg-background text-sm text-foreground focus:outline-none focus:ring-2 focus:ring-ring" required>
            ...
            <div className="space-y-1">
              ...
              <select value={form.categoryId} onChange={set('categoryId')}
                ...
                {filteredCategories.map((c) => (
                  <option key={c.id} value={c.id}>{getCategoryIcon(c)} {c.name}</option>
                ))}
              </select>
            </div>
          )}
          ...
          {error && <p className="text-sm text-rose-400 bg-rose-500/10 rounded-md px-3 py-2">{error}</p>}
