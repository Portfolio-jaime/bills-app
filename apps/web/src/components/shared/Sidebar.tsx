...
const navItems = [
  ...
  { href: '/budgets', label: 'Budgets', icon: PieChart, badge: true },
...
export function Sidebar() {
  ...
  const overBudgetCount = budgets?.filter((b) => (b.percentage ?? 0) >= 80).length ?? 0
          ...
          return (
            ...
            >
              ...
              {badge && overBudgetCount > 0 && (
                <span className="text-xs bg-rose-500 text-white rounded-full px-1.5 py-0.5 min-w-[18px] text-center">
                  {overBudgetCount}
                </span>
              )}
            </Link>
